...
public class diamondhorseshoe extends EpicItem {
	public diamondhorseshoe() {
	...
	private void fall(LivingFallEvent event) {
		if (event.getEntity() instanceof Player player) {
			if (CuriosHandler.hascurio(player, this)) {
				event.setDamageMultiplier(event.getDamageMultiplier() * 0.45f);
			}
		}
	}
	...
	public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> tooltip, TooltipFlag flags) {
		...
		tooltip.add(Component.translatable("-55% 摔落伤害").withStyle(ChatFormatting.GOLD));
		tooltip.add(Component.translatable("-12.5% 受到伤害").withStyle(ChatFormatting.GOLD));
	}
}
