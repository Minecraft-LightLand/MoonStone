...
public class goldhorseshoe extends EpicItem {
	public goldhorseshoe() {
	...
	private void aaa(LivingHurtEvent event) {
		if (event.getEntity() instanceof Player player) {
			if (CuriosHandler.hascurio(player, this)) {
				event.setAmount(event.getAmount() * 0.925f);
			}
		}
	}
	...
	public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> tooltip, TooltipFlag flags) {
		...
		tooltip.add(Component.translatable("-40% 摔落伤害").withStyle(ChatFormatting.GOLD));
		tooltip.add(Component.translatable("-7.5% 受到伤害").withStyle(ChatFormatting.GOLD));
	}
}
