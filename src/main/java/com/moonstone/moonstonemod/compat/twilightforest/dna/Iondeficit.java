...
public class Iondeficit extends Item {
	...
	public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> tooltip, TooltipFlag flags) {
		...
		if (Screen.hasShiftDown()) {
			...
			tooltip.add(Component.translatable("生命值回复速度：+" + (anInt / 20) * 100 + "%").withStyle(ChatFormatting.GOLD));
			tooltip.add(Component.translatable("所有的攻击伤害：+" + anInt / 20 / 4 * 100 + "%").withStyle(ChatFormatting.GOLD));
			tooltip.add(Component.translatable("挖掘速度：+" + (anInt / 20) * 1.33 * 100 + "%").withStyle(ChatFormatting.GOLD));
			tooltip.add(Component.translatable("伤害抗性：+" + anInt / 20 / 3 * 100 + "%").withStyle(ChatFormatting.GOLD));
